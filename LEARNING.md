...
Mục tiêu: xuất phát từ Java/Spring và auth, giờ học thêm để làm backend Rust hiện đại.
Cách chọn tài liệu: ưu tiên đồ ngắn gọn mà chất, đọc xong là dùng được ngay — tránh mấy bài dài lê thê.

Ba chỗ còn yếu, xếp theo độ ưu tiên:
1. Rust ở mức làm thật — yếu nhất nên cày trước.
2. Async messaging (RabbitMQ, transactional outbox, worker).
3. Kubernetes và IaC — để sau cùng, đọc dần là đủ.

Onion và auth thì đã quen (từng làm Spring layered và vài project auth), chỉ cần chỉnh lại tư duy một chút.
